 ...
 *
 * For preferred protocol setting:
 * you can config the the preferred protocol through call the <code>setPreConfigRequest</code>
 * interface. We will try the perferred protocol connection for http or https, and if the
 * preferred connection failed, we will try to use the normal way to connect again.
 */
public final class OkHttpClient implements Cloneable {
  ...
  private Map<String, Protocol> preferredHostProtocol;
  ...
  public synchronized void configPreferredHostProtocol(String host, Protocol p) {
      if (preferredHostProtocol == null) {
          preferredHostProtocol = new HashMap<String, Protocol>();
      }
      preferredHostProtocol.put(host, p);
  }

  public Map<String, Protocol> getPreferredHostProtocol() {
    return preferredHostProtocol;
  }

  public Request preConfigRequest(Request request) {
      // Try the preferred protocol config for host
      if (this.getPreferredHostProtocol() != null
          && this.getPreferredHostProtocol().containsKey(request.url().getHost())) {
        Protocol p = this.getPreferredHostProtocol().get(request.url().getHost());
  ...
  public Call newCall(Request request) {
    ...
    request = preConfigRequest(request);
